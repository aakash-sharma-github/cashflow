...
import {
    ...
    Animated, Pressable, Keyboard, Modal, Platform, ScrollView,
} from 'react-native'
...
// ── Priority config ───────────────────────────────────────────
const PC: Record<Priority, { color: string; bg: string; label: string; icon: string }> = {
    high: { color: COLORS.danger, bg: COLORS.cashOutLight, label: 'High', icon: 'alert-circle' },
    medium: { color: COLORS.warning, bg: '#FFFBEB', label: 'Medium', icon: 'remove-circle' },
    low: { color: COLORS.success, bg: COLORS.cashInLight, label: 'Low', icon: 'checkmark-circle' },
}
...
function DateTimeRow({
    label, value, onChange, theme, iconName,
}: {
    label: string; value: Date | null; onChange: (d: Date | null) => void
    theme: ReturnType<typeof getTheme>; iconName: string
}) {
    ...
    return (
        <View style={{ marginBottom: SPACING.md }}>
            <Text style={[sheetS.label, { color: theme.textSecondary }]}>{label}</Text>
            <View style={{ flexDirection: 'row', gap: SPACING.sm }}>
                <TouchableOpacity
                    style={[sheetS.dateBtn, { backgroundColor: theme.surfaceSecondary, borderColor: value ? COLORS.primary : theme.border }]}
                    onPress={openDate}
                    activeOpacity={0.8}
                >
                    <Ionicons name={iconName as any} size={16} color={value ? COLORS.primary : theme.textTertiary} />
                    <Text style={[sheetS.dateBtnText, { color: value ? COLORS.primary : theme.textTertiary }]}>
            ...
            {!isAndroid && showPicker && (
                <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: SPACING.sm, marginTop: SPACING.sm }}>
                    <TouchableOpacity onPress={() => setShowPicker(false)}>
                        <Text style={{ color: theme.textSecondary, fontSize: FONT_SIZE.sm }}>Cancel</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => { setShowPicker(false); onChange(tempDate) }}>
                        <Text style={{ color: COLORS.primary, fontWeight: '700', fontSize: FONT_SIZE.sm }}>Done</Text>
                    </TouchableOpacity>
                </View>
            )}
        </View>
...
function AddSheet({ visible, onClose, theme }: { visible: boolean; onClose: () => void; theme: ReturnType<typeof getTheme> }) {
    const slideAnim = useSlideAnim(visible)
    const translateY = slideAnim.interpolate({ inputRange: [0, 1], outputRange: [600, 0] })
    const opacity = slideAnim.interpolate({ inputRange: [0, 0.5, 1], outputRange: [0, 1, 1] })
    ...
    return (
        <Animated.View style={[sheetS.overlay, { opacity }]} pointerEvents="box-none">
            <Pressable style={sheetS.backdrop} onPress={() => { Keyboard.dismiss(); onClose() }} />
            <Animated.View style={[sheetS.sheet, { backgroundColor: theme.surface, transform: [{ translateY }] }]}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    <TextInput
                        ref={inputRef}
                        style={[sheetS.input, { backgroundColor: theme.background, borderColor: theme.border, color: theme.text }]}
                        value={text} onChangeText={setText}
                        placeholder="What needs to be done?" placeholderTextColor={theme.textTertiary}
                        multiline maxLength={200}
                    />
                    <Text style={[sheetS.label, { color: theme.textSecondary }]}>Priority</Text>
                    <View style={sheetS.priorityRow}>
                        {(Object.entries(PC) as [Priority, typeof PC.high][]).map(([k, c]) => (
                            <TouchableOpacity
                                key={k}
                                style={[sheetS.priorityChip, { borderColor: theme.border, backgroundColor: theme.background }, priority === k && { borderColor: c.color, backgroundColor: c.bg }]}
                                onPress={() => setPriority(k)} activeOpacity={0.8}
                            >
                                <Ionicons name={c.icon as any} size={14} color={priority === k ? c.color : theme.textTertiary} />
                                <Text style={[sheetS.priorityText, { color: priority === k ? c.color : theme.textSecondary }]}>{c.label}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                    ...
                    <Text style={[sheetS.label, { color: theme.textSecondary }]}>Notes (optional)</Text>
                    <TextInput
                        style={[sheetS.notesInput, { backgroundColor: theme.background, borderColor: theme.border, color: theme.text }]}
                        value={notes} onChangeText={setNotes}
                        placeholder="Add details..." placeholderTextColor={theme.textTertiary}
                        multiline maxLength={500}
                    />
                </ScrollView>
                <View style={sheetS.actions}>
                    <TouchableOpacity style={[sheetS.cancelBtn, { borderColor: theme.border }]} onPress={onClose}>
                        <Text style={[sheetS.cancelText, { color: theme.textSecondary }]}>Cancel</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[sheetS.confirmBtn, !text.trim() && { opacity: 0.5 }]} onPress={handleAdd} disabled={!text.trim()} activeOpacity={0.88}>
                        <LinearGradient colors={['#5B5FED', '#7C3AED']} style={sheetS.confirmGrad}>
...
function EditSheet({ todo, onClose, theme }: { todo: Todo | null; onClose: () => void; theme: ReturnType<typeof getTheme> }) {
    const visible = !!todo
    const slideAnim = useSlideAnim(visible)
    const translateY = slideAnim.interpolate({ inputRange: [0, 1], outputRange: [600, 0] })
    const opacity = slideAnim.interpolate({ inputRange: [0, 0.5, 1], outputRange: [0, 1, 1] })
    ...
    const handleSave = async () => {
        ...
        if (todo.reminderDate) await notificationService.cancelTaskReminder(todo.id)
        await useTodoStore.getState().updateTodo(todo.id, {
        ...
        <Animated.View style={[sheetS.overlay, { opacity }]} pointerEvents="box-none">
            ...
            <Animated.View style={[sheetS.sheet, { backgroundColor: theme.surface, transform: [{ translateY }] }]}>
                ...
                <View style={sheetS.actions}>
                    ...
                    <TouchableOpacity style={[sheetS.confirmBtn, !text.trim() && { opacity: 0.5 }]} onPress={handleSave} disabled={!text.trim()} activeOpacity={0.88}>
                        <LinearGradient colors={['#5B5FED', '#7C3AED']} style={sheetS.confirmGrad}>
...
// ── Task Preview Modal ─────────────────────────────────────────
function TaskPreview({ todo, onClose, onEdit, theme }: { todo: Todo | null; onClose: () => void; onEdit: () => void; theme: ReturnType<typeof getTheme> }) {
    if (!todo) return null
    ...
    return (
        <Modal transparent visible={!!todo} animationType="fade" onRequestClose={onClose}>
                ...
                <View style={previewS.body}>
                    ...
                    <View style={previewS.header}>
                        <View style={[previewS.priorityPill, { backgroundColor: pc.bg }]}>
                            ...
                            <Text style={[previewS.priorityText, { color: pc.color }]}>{pc.label} Priority</Text>
                        </View>
                    ...
                    {/* Task text */}
                    <Text style={[previewS.taskText, { color: theme.text }, todo.completed && { textDecorationLine: 'line-through', color: theme.textTertiary }]}>
                        {todo.text}
                    </Text>

                    {/* Status badge */}
                    <View style={[previewS.statusBadge, { backgroundColor: todo.completed ? COLORS.cashInLight : theme.surfaceSecondary }]}>
                        <Ionicons name={todo.completed ? 'checkmark-circle' : 'time-outline'} size={13} color={todo.completed ? COLORS.cashIn : theme.textSecondary} />
                        <Text style={[previewS.statusText, { color: todo.completed ? COLORS.cashIn : theme.textSecondary }]}>
                            {todo.completed ? `Completed ${todo.completedAt ? format(parseISO(todo.completedAt), 'dd MMM, h:mm a') : ''}` : 'Active'}
                        </Text>
                    </View>
                    ...
                    {due && (
                        <View style={previewS.metaRow}>
                            <Ionicons name="calendar-outline" size={14} color={due.overdue ? COLORS.cashOut : theme.textTertiary} />
                            <Text style={[previewS.metaLabel, { color: theme.textSecondary }]}>Due</Text>
                            <Text style={[previewS.metaVal, { color: due.overdue ? COLORS.cashOut : theme.text }]}>
                    ...
                    {todo.notes && (
                        <View style={[previewS.notesBox, { backgroundColor: theme.background }]}>
                            <Text style={[previewS.notesLabel, { color: theme.textTertiary }]}>Notes</Text>
                            <Text style={[previewS.notesText, { color: theme.text }]}>{todo.notes}</Text>
                        </View>
                    )}
                    ...
                    <View style={previewS.actions}>
                        <TouchableOpacity
                            style={[previewS.toggleBtn, { backgroundColor: todo.completed ? theme.surfaceSecondary : COLORS.cashInLight }]}
                            onPress={() => { useTodoStore.getState().toggleTodo(todo.id); onClose() }}
                        >
                            <Ionicons name={todo.completed ? 'refresh-outline' : 'checkmark-circle-outline'} size={16} color={todo.completed ? theme.textSecondary : COLORS.cashIn} />
                            <Text style={[previewS.toggleText, { color: todo.completed ? theme.textSecondary : COLORS.cashIn }]}>
                                {todo.completed ? 'Mark Active' : 'Mark Done'}
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[previewS.editBtn, { backgroundColor: COLORS.primaryLight }]} onPress={onEdit}>
                            <Ionicons name="pencil-outline" size={16} color={COLORS.primary} />
                            <Text style={[previewS.editText, { color: COLORS.primary }]}>Edit</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
...
const TodoItem = memo(function TodoItem({
    item, onToggle, onThreeDot, onPress, theme,
}: {
    item: Todo; onToggle: () => void; onThreeDot: () => void; onPress: () => void; theme: ReturnType<typeof getTheme>
}) {
    ...
    return (
        <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
            <Pressable
                style={({ pressed }) => [s.todoCard, { backgroundColor: theme.surface }, pressed && { opacity: 0.9 }, item.completed && { opacity: 0.6 }]}
                onPress={onPress}
            ...
            >
                ...
                <TouchableOpacity style={s.checkWrap} onPress={handleToggle} activeOpacity={0.7}>
                    <View style={[s.checkbox, { borderColor: item.completed ? pc.color : theme.border }, item.completed && { backgroundColor: pc.color }]}>
                        {item.completed && <Ionicons name="checkmark" size={12} color="#fff" />}
                    </View>
                </TouchableOpacity>
                <View style={s.todoContent}>
                    <Text style={[s.todoText, { color: theme.text }, item.completed && { textDecorationLine: 'line-through', color: theme.textTertiary }]} numberOfLines={2}>
                        {item.text}
                    </Text>
                    <View style={s.todoMeta}>
                        ...
                        {due && (
                            <View style={[s.dueBadge, { backgroundColor: due.overdue ? '#FEF2F2' : theme.surfaceSecondary }]}>
                                <Ionicons name="calendar-outline" size={10} color={due.overdue ? COLORS.cashOut : theme.textTertiary} />
                                <Text style={[s.dueText, { color: due.overdue ? COLORS.cashOut : theme.textTertiary }]}>{due.text}</Text>
                            </View>
                        )}
                        {hasReminder && (
                            <View style={[s.dueBadge, { backgroundColor: COLORS.primaryLight }]}>
                        ...
                        {item.notes && <Ionicons name="document-text-outline" size={12} color={theme.textTertiary} />}
                    </View>
                </View>
                {/* Three-dot menu */}
                <TouchableOpacity
...
export default function TodoScreen() {
    const todos_raw = useTodoStore(s => s.todos)
    const filter = useTodoStore(s => s.filter)
    const searchQuery = useTodoStore(s => s.searchQuery)
    const isLoaded = useTodoStore(s => s.isLoaded)
    ...
    const handleToggle = useCallback(async (item: Todo) => {
        ...
        if (!item.completed && item.reminderDate) {
            ...
            await useTodoStore.getState().updateTodo(item.id, { reminderNoteId: null, reminderDueId: null })
        }
    }, [])
    ...
    const handleThreeDot = useCallback((item: Todo) => {
        themedActionSheet(item.text, undefined, [
            ...
            },
            {
                ...
                onPress: () => themedAlert('Delete Task', `"${item.text}" will be permanently removed.`, [
                    { text: 'Cancel', style: 'cancel' },
                    {
                        text: 'Delete', style: 'destructive', onPress: async () => {
                            await notificationService.cancelTaskReminder(item.id)
    ...
    // Memoised computed values — only recompute when todos/filter/search change
    const todos = useMemo(() => {
        let result = todos_raw
    ...
    }, [todos_raw, filter, searchQuery])

    const stat = useMemo(() => ({
        total: todos_raw.length,
        completed: todos_raw.filter(t => t.completed).length,
        active: todos_raw.filter(t => !t.completed).length,
        high: todos_raw.filter(t => t.priority === 'high' && !t.completed).length,
    }), [todos_raw])
        ...
        <SafeAreaView style={[s.container, { backgroundColor: theme.background }]}>
            ...
            <View style={s.header}>
                <View>
                    ...
                    <Text style={[s.headerSub, { color: theme.textSecondary }]}>{stat.active} remaining · {stat.completed} done</Text>
                </View>
                <View style={s.headerActions}>
                    <TouchableOpacity style={[s.headerBtn, { backgroundColor: theme.surface }]} onPress={() => setSearchVisible(v => !v)}>
                        <Ionicons name={searchVisible ? 'close' : 'search'} size={18} color={theme.textSecondary} />
                    </TouchableOpacity>
                    {stat.completed > 0 && (
                        <TouchableOpacity style={[s.headerBtn, { backgroundColor: theme.surface }]} onPress={() => {
                            themedAlert('Clear Completed', `Remove all ${stat.completed} completed tasks?`, [
                                { text: 'Cancel', style: 'cancel' },
            ...
            {searchVisible && (
                <View style={[s.searchWrap, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                    ...
                    <TextInput
                        ...
                        placeholder="Search tasks..." placeholderTextColor={theme.textTertiary} autoFocus
                    />
            ...
            <View style={[s.filterBar, { backgroundColor: theme.background }]}>
                {FILTERS.map(f => {
                    ...
                    return (
                        <TouchableOpacity
                            key={f.key}
                            style={[s.filterTab, { backgroundColor: theme.surface, borderColor: theme.border }, active && { backgroundColor: COLORS.primary, borderColor: COLORS.primary }]}
                            onPress={() => setFilter(f.key)} activeOpacity={0.8}
                        >
                            <Text style={[s.filterTabText, { color: active ? '#fff' : theme.textSecondary }]}>{f.label}</Text>
                            {count > 0 && (
                                <View style={[s.filterCount, { backgroundColor: active ? 'rgba(255,255,255,0.25)' : theme.surfaceSecondary }]}>
                                    <Text style={[s.filterCountText, { color: active ? '#fff' : theme.textTertiary }]}>{count}</Text>
                                </View>
                            )}
                        </TouchableOpacity>
                    )
            ...
            {stat.high > 0 && filter !== 'completed' && (
                <View style={[s.urgentBanner, { backgroundColor: COLORS.cashOutLight }]}>
                    <Ionicons name="alert-circle" size={14} color={COLORS.danger} />
                    <Text style={[s.urgentText, { color: COLORS.danger }]}>{stat.high} high-priority task{stat.high > 1 ? 's' : ''} pending</Text>
                </View>
            )}
                ...
                ListEmptyComponent={
                    <View style={s.empty}>
                        ...
                        <Text style={[s.emptySub, { color: theme.textSecondary }]}>
                            {filter === 'completed' ? 'Complete a task to see it here.' : filter === 'active' ? 'No active tasks. Add one below.' : 'Tap + to add your first task.'}
                        </Text>
                    </View>
                }
            />
            ...
            {/* FAB */}
            <TouchableOpacity style={s.fab} onPress={() => setAddVisible(true)} activeOpacity={0.9}>
            ...
            {/* Sheets */}
            {addVisible && <AddSheet visible={addVisible} onClose={() => setAddVisible(false)} theme={theme} />}
            {editTodo && <EditSheet todo={editTodo} onClose={() => setEditTodo(null)} theme={theme} />}

            {/* Preview */}
            <TaskPreview
                todo={previewTodo}
                onClose={() => setPreviewTodo(null)}
                onEdit={() => { setEditTodo(previewTodo); setPreviewTodo(null) }}
                theme={theme}
            />
        </SafeAreaView>
    )
...
const sheetS = StyleSheet.create({
    ...
    sheet: { borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingHorizontal: SPACING.lg, paddingBottom: 40, maxHeight: '90%', ...SHADOW.lg },
    handle: { width: 40, height: 4, borderRadius: 2, alignSelf: 'center', marginTop: 12, marginBottom: SPACING.md },
    title: { fontSize: FONT_SIZE.xl, fontWeight: '800', marginBottom: SPACING.md },
    editHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: SPACING.md },
    input: { borderRadius: BORDER_RADIUS.lg, borderWidth: 1.5, padding: SPACING.md, fontSize: FONT_SIZE.md, minHeight: 80, textAlignVertical: 'top', marginBottom: SPACING.lg },
    notesInput: { borderRadius: BORDER_RADIUS.lg, borderWidth: 1.5, padding: SPACING.md, fontSize: FONT_SIZE.md, minHeight: 60, textAlignVertical: 'top', marginBottom: SPACING.lg },
    ...
    dateBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: SPACING.md, paddingVertical: 11, borderRadius: BORDER_RADIUS.lg, borderWidth: 1.5 },
    dateBtnText: { fontSize: FONT_SIZE.sm, fontWeight: '500' },
    clearBtn: { width: 38, height: 38, borderRadius: BORDER_RADIUS.md, alignItems: 'center', justifyContent: 'center' },
    reminderHint: { flexDirection: 'row', alignItems: 'center', gap: 6, padding: SPACING.sm, borderRadius: BORDER_RADIUS.md, marginBottom: SPACING.md },
    reminderHintText: { fontSize: FONT_SIZE.xs, flex: 1 },
    actions: { flexDirection: 'row', gap: SPACING.sm, paddingTop: SPACING.sm },
    cancelBtn: { flex: 1, paddingVertical: 13, alignItems: 'center', borderRadius: BORDER_RADIUS.lg, borderWidth: 1.5 },
    cancelText: { fontSize: FONT_SIZE.md, fontWeight: '600' },
    confirmBtn: { flex: 2, borderRadius: BORDER_RADIUS.lg, overflow: 'hidden' },
    confirmGrad: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 13, gap: 6 },
...
const previewS = StyleSheet.create({
    ...
    card: { margin: SPACING.lg, borderRadius: 24, overflow: 'hidden', ...SHADOW.lg },
    accentBar: { height: 5 },
    body: { padding: SPACING.lg },
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: SPACING.md },
    priorityPill: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
    priorityText: { fontSize: FONT_SIZE.xs, fontWeight: '700' },
    taskText: { fontSize: FONT_SIZE.xl, fontWeight: '700', lineHeight: 26, marginBottom: SPACING.md },
    statusBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 6, borderRadius: BORDER_RADIUS.md, alignSelf: 'flex-start', marginBottom: SPACING.md },
    ...
    notesBox: { borderRadius: BORDER_RADIUS.md, padding: SPACING.md, marginTop: SPACING.sm, marginBottom: SPACING.sm },
    notesLabel: { fontSize: FONT_SIZE.xs, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4 },
    notesText: { fontSize: FONT_SIZE.sm, lineHeight: 20 },
    createdAt: { fontSize: FONT_SIZE.xs, marginTop: SPACING.sm, marginBottom: SPACING.lg },
    actions: { flexDirection: 'row', gap: SPACING.sm },
    toggleBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 11, borderRadius: BORDER_RADIUS.lg },
    toggleText: { fontSize: FONT_SIZE.sm, fontWeight: '600' },
    editBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 11, borderRadius: BORDER_RADIUS.lg },
...
const s = StyleSheet.create({
    ...
    dueBadge: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingHorizontal: 7, paddingVertical: 2, borderRadius: 10 },
