...
export default function ExportImportScreen({ route }: any) {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      themedAlert("Export Failed", e.message);
    } finally {
      setCsvLoading(false);
    }
  };
  ...
  const handlePickCSV = async () => {
    ...
    try {
      ...
      themedAlert("Could not read file", e.message);
    } finally {
          ...
          onPress: async () => {
            ...
            for (const r of importPreview.rows) {
              const { error } = await entriesService.createEntry(bookId, {
                ...
                type: r.type,
                note: r.note || "",
                entry_date: new Date(r.entry_date),
              });
              if (error) fail++;
              else ok++;
            }
            ...
            themedAlert(
              "Import Complete ✅",
              `${ok} entr${ok === 1 ? "y" : "ies"} imported successfully.${fail > 0 ? `\n${fail} failed.` : ""}`,
            );
          },
        },
      ],
    ...
    >
      ...
      >
        ...
        <View style={s.sectionHead}>
          <View
            style={[s.sectionIconWrap, { backgroundColor: COLORS.cashInLight }]}
          >
          ...
          <View>
            ...
            <Text style={[s.sectionSub, { color: theme.textSecondary }]}>
              Download entries from this book
            </Text>
          </View>
        </View>
        ...
        <ActionCard
          icon="grid-outline"
          iconBg={COLORS.cashInLight}
          iconColor={COLORS.cashIn}
          title="Export as CSV"
          desc="Compatible with CashBook, Excel, Google Sheets"
        ...
        <ActionCard
          ...
          title="Export as PDF"
          desc="Branded report with summary and transaction table"
        ...
        <View style={s.codeCard}>
          ...
          <Text style={s.codeSubLabel}>
            Format A — CashBook / this app's export:
          </Text>
          <Text style={s.codeLine}>
            Date,Time,Remark,Entry by,Cash In,Cash Out,Balance
          </Text>
          <Text style={s.codeLine}>
            03-21-26,13:23,Salary,Aakash,108000,,108000
          </Text>
          <Text style={s.codeLine}>
            03-25-26,21:01,Groceries,Aakash,,1700,106300
          </Text>
          ...
          <Text style={s.codeLine}>2024-01-15,Cash In,5000,Salary</Text>
          <Text style={s.codeLine}>2024-01-16,Cash Out,1700,Groceries</Text>

          <Text style={s.codeNote}>
            Balance column is ignored on import · Entry by is informational only
          </Text>
        </View>
        ...
        <ActionCard
          ...
          desc="Pick a .csv file — Format A or B both work"
          ...
          >
            ...
            <View style={[s.statsRow, { backgroundColor: theme.background }]}>
              <View style={s.statItem}>
                <Text style={[s.statNum, { color: theme.text }]}>
              ...
              <View
                style={[s.statDivider, { backgroundColor: theme.border }]}
              />
              <View style={s.statItem}>
            ...
            {importPreview.skipped > 0 && (
              <View style={s.warnRow}>
                ...
                <Text style={s.warnText}>
                  {" "}
                  {importPreview.skipped} row
                  {importPreview.skipped > 1 ? "s" : ""} skipped (invalid or
                  empty)
                </Text>
              </View>
            )}
              ...
              <View style={[s.errorBox, { backgroundColor: "#fef2f2" }]}>
                ...
                {importPreview.errors.length > 3 && (
                  <Text style={s.errorText}>
                    … and {importPreview.errors.length - 3} more
                  </Text>
                )}
              </View>
            )}
            ...
            {importPreview.rows.length > 0 && (
              <View style={s.rowPreviewList}>
                <Text
                  style={[s.rowPreviewHeading, { color: theme.textSecondary }]}
                >
                  ...
                  >
                    ...
                    >
                      <Ionicons
                        ...
                        size={10}
                        color={
                          r.type === "cash_in" ? COLORS.cashIn : COLORS.cashOut
                        }
                      />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[s.rowPreviewNote, { color: theme.text }]}
                        numberOfLines={1}
                      >
                        {r.note ||
                ...
                {importPreview.rows.length > 5 && (
                  <Text
                    style={[s.rowPreviewMore, { color: theme.textTertiary }]}
                  >
            ...
            {importPreview.rows.length > 0 ? (
              <View style={s.previewActions}>
                ...
                >
                  <Text
                    style={[s.cancelBtnText, { color: theme.textSecondary }]}
                  >
                    Cancel
                  </Text>
                </TouchableOpacity>
              ...
              <Text style={[s.noRows, { color: theme.textSecondary }]}>
                No valid rows found. Check the file matches one of the formats
                above.
              </Text>
            )}
          </View>
