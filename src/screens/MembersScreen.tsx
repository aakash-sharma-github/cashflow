...
export default function MembersScreen({ route }: any) {
  ...
  const renderMember = ({ item: m }: { item: BookMember }) => (
    <TouchableOpacity
      style={[s.row, { backgroundColor: theme.surface }]}
    ...
    >
      <LinearGradient
        ...
        style={s.avatar}
      >
        <Text style={s.avatarText}>{getInitials(m.profile?.full_name || m.profile?.email || '?')}</Text>
      </LinearGradient>
      <View style={s.rowInfo}>
        <Text style={[s.rowName, { color: theme.text }]}>
          {getDisplayName(m.profile)}{m.user_id === user?.id ? ' (You)' : ''}
        </Text>
        <Text style={[s.rowEmail, { color: theme.textTertiary }]}>{m.profile?.email}</Text>
      </View>
      <View style={[s.rolePill, { backgroundColor: m.role === 'owner' ? COLORS.primaryLight : theme.surfaceSecondary }]}>
        {m.role === 'owner' && <Ionicons name="star" size={11} color={COLORS.primary} />}
        <Text style={[s.roleText, { color: m.role === 'owner' ? COLORS.primary : theme.textSecondary }]}>
  ...
  const renderInvite = ({ item: inv }: { item: Invitation }) => {
    ...
    return (
      <View style={[s.row, { backgroundColor: theme.surface }]}>
        <View style={[s.avatar, { backgroundColor: theme.border, borderRadius: 22 }]}>
          <Text style={[s.avatarText, { color: theme.textSecondary }]}>
            {getInitials(inv.invitee_email)}
          </Text>
        </View>
        <View style={s.rowInfo}>
          <Text style={[s.rowName, { color: theme.text }]} numberOfLines={1}>{inv.invitee_email}</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            <Ionicons name="time-outline" size={11} color={theme.textTertiary} />
            <Text style={[s.rowEmail, { color: theme.textTertiary }]}>{formatRelativeTime(inv.created_at)}</Text>
          </View>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.sm }}>
          ...
          {inv.status === 'pending' && isOwner && (
            <TouchableOpacity onPress={async () => { await invitationsService.cancelInvitation(inv.id); load() }}>
  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: theme.background }]} edges={['bottom']}>
      {/* Invite bar */}
      {isOwner && (
        <View style={[s.inviteBar, { backgroundColor: theme.surface, borderBottomColor: theme.border }]}>
          <Text style={[s.inviteLabel, { color: theme.textSecondary }]}>Invite by Email</Text>
          <View style={s.inviteRow}>
            ...
            <TouchableOpacity
              ...
              disabled={sending}
            >
              <LinearGradient colors={['#5B5FED', '#7C3AED']} style={s.sendBtnGrad}>
                {sending
        ...
        {(['members', 'invitations'] as TabKey[]).map(t => {
          ...
          return (
            ...
            >
              ...
              />
              <Text style={[s.tabLabel, { color: active ? COLORS.primary : theme.textSecondary }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="mail-open-outline" size={40} color={theme.textTertiary} />
              <Text style={[s.emptyText, { color: theme.textSecondary }]}>No invitations sent yet</Text>
            </View>
          }
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.primary} />}
        />
...
const s = StyleSheet.create({
  ...
  inviteLabel: { fontSize: FONT_SIZE.xs, fontWeight: '700', marginBottom: SPACING.sm, textTransform: 'uppercase', letterSpacing: 0.6 },
  inviteRow: { flexDirection: 'row', gap: SPACING.sm },
  inviteInputWrap: { flex: 1, flexDirection: 'row', alignItems: 'center', borderRadius: BORDER_RADIUS.md, borderWidth: 1.5, paddingHorizontal: SPACING.md, height: 44 },
  ...
  tabBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 9, borderRadius: BORDER_RADIUS.md, borderWidth: 1.5 },
  tabLabel: { fontSize: FONT_SIZE.sm, fontWeight: '600' },

  list: { paddingHorizontal: SPACING.md, paddingBottom: 40 },
  listFlex: { flex: 1 },

  row: { flexDirection: 'row', alignItems: 'center', borderRadius: BORDER_RADIUS.lg, padding: SPACING.md, marginBottom: 8, ...SHADOW.sm },
